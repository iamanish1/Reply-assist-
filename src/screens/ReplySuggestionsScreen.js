...
export function ReplySuggestionsScreen({ navigation }) {
  ...
  return (
    <Screen
      bottom={
        <View style={styles(t).bottomRow}>
          <View style={styles(t).bottomLeft}>
            <AppButton
          ...
          <View style={styles(t).bottomRight}>
            <AppButton
    ...
    >
      ...
      <View style={styles(t).container}>
        <AppText variant="small" style={styles(t).sub}>
          Pick one that feels like you.
        </AppText>

        <View style={styles(t).list}>
          {replies.slice(0, 3).map((r, idx) => (
            <View key={`${r.tone}-${idx}`} style={styles(t).item}>
